...
def write_headline(
    ...
    """
    Generate a short newspaper-style Bengali headline for the photo card.

    Returns the headline text, or None if the AI call fails (caller falls
    back to the raw scraped headline).
    """
    body_snippet = (article.get("body_text") or "")[:1500]
    user_content = (
        f"{headline_prompt}\n\n"
    ...
    try:
        ...
        if not text:
            log.error(
                "Headline AI returned empty content for '%s'",
                article["headline"][:70],
            )
            return None
        text = text.replace("—", ",").strip().strip('"').strip("।").strip()
        log.info("Card headline written: %s", text)
        return text
    except Exception as exc:
        log.error(
            "Headline AI call failed for '%s': %s",
            article["headline"][:70],
            exc,
        )
        return None
